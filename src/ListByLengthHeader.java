 ...
 * for each list of words by length. It will store length of the words in the
 * list, an array of characters in order of most popular, and a linkedlist of
 * strings which will be all the words of that length.
 */
class ListByLengthHeader
{
	...
	public char getMax(List<Integer> alreadyGuessed)
	{
		...
		// initially max is 0 (NOT charCounts[0] or else the max will default to
		// count of letter a)
		int max = 0;
	...
	public void reduceIncluded(char c, List<Integer> positions)
	{
		...
		for (int i = 0; i < words.length; i++)
		{
			...
			{
				// if word at index i doesnt have that letter at that position,
				// remove it
				if (words[i].charAt(pos) != c)
				{
					...
					// we don't need to check rest of positions, just break and
					// go to next word
					break;
				}
			}
